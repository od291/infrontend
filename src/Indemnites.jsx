...
function Indemnites() {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      const url = modifyId ? `https://inbackend-production.up.railway.app/indemnites/${modifyId}` : 'https://inbackend-production.up.railway.app/indemnites';
  ...
  const fetchIndemnitesByType = (type) => {
    fetch(`https://inbackend-production.up.railway.app/indemnites/type/${type}`)
      .then(response => response.json())
      .then(data => {
        setIndemnitesData(data);
      })
